...
- **Letterboxd tab: background fetch + caching** *(confirmed bug — high priority)* — Every click on the Letterboxd tab triggers a full re-fetch of all saved RSS URLs, causing a visible "Loading…" hang on every navigation. `GET /api/letterboxd/movies` fetches all URLs synchronously at request time — with curator accounts expanding into many lists + FlareSolverr (70s timeout) this is severe. Fix: cache the last successful fetch result (movies + scores) in a sidecar file (e.g. `data/letterboxd_cache.json`), serve the cache immediately on page load, and only re-fetch in the background (triggered on URL add/remove, manual ↻, or a TTL). Show "last updated X min ago" timestamp in the UI.
