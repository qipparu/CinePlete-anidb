"""
Cineplete — Emby scanner
-------------------------
Drop-in replacement for jellyfin_api.py / plex_xml.py.
Returns the exact same tuple: (plex_ids, directors, actors, stats, no_tmdb_guid)
so scanner.py needs no logic changes.

Emby and Jellyfin share the same REST API surface (Jellyfin forked from Emby
in 2018). The only structural difference is that Emby requires an /emby/ prefix
on every path:
    Jellyfin:  http://host:8096/Items
    Emby:      http://host:8096/emby/Items
"""
...
# Path prefix required by all Emby REST endpoints
_EMBY_PREFIX = "/emby"


def _build_lib_cfg(lib_cfg):
    """Resolve lib_cfg — if None, fall back to legacy EMBY config section."""
...
def _emby_get(path: str, lib_cfg=None, params: dict = None, timeout: int = 120) -> dict:
...
def _library_id(library_name: str, lib_cfg=None) -> str:
    """Resolve a library name to its Emby item ID."""
    data = _emby_get("/Library/MediaFolders", lib_cfg)
...
def scan_movies(lib_cfg=None):
    """
    Scan the configured Emby movie library.
    ...
    log.info(f"Emby library '{library_name}' → ID {library_id}")
    ...
    while True:
        ...
        for item in items:
            ...
            # TMDB ID — stored as PascalCase "Tmdb" in ProviderIds
            # Fall back to lowercase "tmdb" defensively
            ...
            # Limit actors to top 5 per film — Emby returns ALL cast
